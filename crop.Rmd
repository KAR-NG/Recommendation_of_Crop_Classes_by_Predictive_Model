...
In previous decision tree which I grown a tree, and now I grow a forest. It means that I build multiple trees from the data set to achieve a better predictive performance. 

I tune a couple of hyperparameters in the following codes. These tunings may help to avoid overfitting on noisy data set (P. Bruce and Bruce 2017). 
...
4 different nodesize (1, 2, 4, 8) produced a 99% of accuracy. Nodesize is the minimum of terminal nodes. The differences in term of accuracy between nodesize were really small, node 8 has the highest median accuracy of 99.7% base on cross validations. 

I will pick nodesize at 8 to be the most appropriate nodesize for this random forest modeling.
...
importance(model_rf2$`8`$finalModel)
